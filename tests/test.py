...
'''
Test script
*WARNING* Don't run this on a production neoscoin server! *WARNING*
...
import neoscoinrpc
from neoscoinrpc.exceptions import neoscoinException, InsufficientFunds
...
if __name__ == "__main__":
    ...
    if args.config:
        from neoscoinrpc.config import read_config_file
    ...
    connections = []
    if not args.nolocal:
        local_conn = neoscoinrpc.connect_to_local()  # will use read_default_config
        connections.append(local_conn)
    if not args.noremote:
        remote_conn = neoscoinrpc.connect_to_remote(
    ...
    for conn in connections:
        ...
        neoscoinaddress = conn.getnewaddress(account)
        conn.setaccount(neoscoinaddress, account)
        ...
        #conn.sendtoaddress(neoscoinaddress, amount, comment=None, comment_to=None)
        conn.getreceivedbyaddress(neoscoinaddress)
        ...
        for message in messages:
            signature = conn.signmessage(neoscoinaddress, message)
            assert(conn.verifymessage(neoscoinaddress, signature, message) is True)
        ...
        try:
            conn.sendtoaddress(neoscoinaddress, 10000000)
